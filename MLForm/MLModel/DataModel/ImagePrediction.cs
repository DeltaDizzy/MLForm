...
namespace MLtest.DataModel
{
    public class ImagePrediction
    {
        ...
        private static List<Result> NMS(List<float[]> postProcessedBoundBoxes, string[] categories)
        {
            ...
            int counter = 0;
            while (counter < postProcessedBoundBoxes.Count)
            {
                var result = postProcessedBoundBoxes[counter];
                if (result == null)
                {
                    counter++;
                    continue;
                ...
                postProcessedBoundBoxes[counter] = null;

                var iou = postProcessedBoundBoxes.Select(bbox => bbox == null ? float.NaN : BoxIoU(result, bbox)).ToList();
                ...
                for (int i = 0; i < iou.Count; i++)
                {
                    ...
                    {
                        postProcessedBoundBoxes[i] = null;
                    }
                }
                counter++;
            }
